...
class Prc_krs_fak extends CI_Controller {
	...
	public function index()
	{
		echo "
			...
			<body>
				<form method='POST' action='".base_url('wawan/prc_krs_fak/prc_krs')."'>
	...
	{
		$dataLama = $this->Prc->krsLamaFak($this->input->post('fakultas'));
		$dataKrsLama = array();
		...
		foreach ($dataKrsLama as $show) {
			...
			if($tahun>='20101'){
				...
				$check_krs=$this->Prc->check_krs($show['NIM'],$show['Tahun'],$show['IDJadwal'],$show['KodeMK']);
				...
				if($check_krs==0){
				    ...
				    //print_r($data);
					$cek_simpan=$this->Prc->addData($data,$show['Tahun']);
			        if($cek_simpan=='0'){
