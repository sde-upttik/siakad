...
class Prc_krs_TR extends CI_Controller {
	...
	public function index($thn='',$fak='')
	{
		echo "
			...
			<body>
				".$fak." - ".$thn."<br>
				<form method='POST' action='".base_url('wawan/prc_krs_TR/prc_krs')."'>
	...
	{
		$this->index($this->input->post('tahun'),$this->input->post('fakultas'));
		$dataLama = $this->Prc->krsLamaTR($this->input->post('tahun'),$this->input->post('fakultas'));
		$dataKrsLama = array();
		...
		foreach ($dataKrsLama as $show) {
			...
			$cek_simpan=$this->Prc->addDataTR($data,$this->input->post('tahun'));
	        if($cek_simpan=='0'){
