...
class Jdwlkuliah1 extends CI_Controller {
	...
	private function rediralert($type, $pesan, $list, $thn, $Program, $kdj){
		if (empty($list)){
			...
			$value = $this->redirtampiljadwal($thn, $Program, $kdj);
	...
	private function redirtampiljadwal($thn, $kdp, $kdj){
		$ulevel = $this->session->userdata('ulevel');
		...
		foreach ($r as $row) {
			...
			*/
			...
			$mhsw2 = $this->HitungMhsw_fd($IDJADWAL,$thn,$kdj);
			...
			if ($prn == 0) {
				...
				if($stjdw==''){
					$streditmk = "<button type='button' onClick='action($int)' class='btn btn-$icjdw' data-toggle='modal' data-target='#modal-add-jadwal'><img src='".base_url()."assets/images/edit.png' border=0></button><input id='act_name$int' value='$mk' hidden=true readonly /><input id='act_value$int' value='".$jid."' hidden=true readonly /><br><a onClick='GetListKelasKuliah()'>...</a>";
	...
	public function HitungMhsw($jid,$thn,$kdj) {
		...
		if($thn!='20161'){*/
			$s = "select count(*) as JML from _v2_krs$thn where IDJadwal='$jid' and Tahun='$thn' and st_wali=1"; // Modif 08 - 2006 and Tahun='$thn'
		//}
	...
	function daftarmhsnil() {
		...
		if ($pesan){
			...
			$jur == "P101" or
			$jur == "K2MF111" or $jur == "K2MC201" or $jur == "K2ME281" or
