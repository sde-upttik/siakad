...
<body>
	<?php 
		foreach ($mhsw as $show) {
			...
			/*if($show->st_feeder<=0 AND $st_feederkrs<=0){
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12 text-center" style="font-size: 17px; font-weight: bold;">
						...
						<br>Semester 
							<?php 
								if(substr($semester, 4,1)=='3' or substr($semester, 4,1)=='4'){
									echo substr($semester, 0,4)."/".(substr($semester, 0,4)+1)." Antara";
								}else{
									echo $controller->getTahun($show->NIM, $semester);
								}
							?>
					</div>
				</div><br><br>
						...
						<table border="1" style="border: 1px solid black; width: 100%">
							...
							<?php
								...
								foreach ($controller->detailKhs($semester, $show->NIM) as $show1) {
									...
									$KodeMK=$show1->KodeMK;
									$MK=$show1->NamaMK;
									if (empty($MK)) $MK = $controller->getMatakuliah($KodeMK,$kdj);
							?>
									<tr>
